...
| **6** | `flock_shape.py` | **Flock shape analysis** — PCA of the position covariance to extract aspect ratio, orientation, and convex-hull area. Interpolates Young et al.'s shape-driven m*: ~6.05 for thin/longitudinal flocks, ~9.78 for thick/transverse flocks. Closed-form 2×2 eigensolve. |
| **6** | `h2_robustness.py` | **H₂ robustness metric** — Young et al. (2013) consensus robustness via Laplacian eigenvalues of the k-NN interaction graph. Jacobi eigenvalue solver (pure Python, no numpy). η(m) per-neighbour efficiency. Cost-optimal m* balancing robustness against O(m) sensing cost. |
